...
final class ReaderModel: ObservableObject {
    ...
    init(
        ...
        self.bookSourceEngine = bookSourceEngine ?? LegadoBookSourceEngine()
    ...
    var chapterProgressText: String {
        ...
        return "章节 \(selectedChapterIndex + 1) / \(max(chapters.count, 1))"
    }
    ...
    var selectedChapterDisplayText: String {
        ...
        return "\(selectedChapterIndex + 1) / \(max(chapters.count, 1))"
    }
    ...
    private func openOnlineBook(book: Book) async {
        guard case .online(let sourceId, let bookUrl) = book.origin,
              let source = bookSources.first(where: { $0.id == sourceId }) else {
            errorMessage = "找不到对应的书源。"
            return
        }
        ...
        selectedChapterIndex = min(book.lastReadPosition.chapterIndex, 0)
        activePanel = nil
        exitBossKeyRecording()
        screen = .reader
        ...
        do {
            let chapters = try await bookSourceEngine.loadChapterList(bookUrl: bookUrl, source: source)
            onlineChapters = chapters
            provider.preloadChapterList(chapters, for: book.id)
            if selectedChapterIndex >= chapters.count { selectedChapterIndex = 0 }
    ...
    func loadOnlineChapterContent(at index: Int) async {
